...
describe("manual ingestion status", () => {
  ...
  it("searches only after the exact vehicle manual is ready and renders sources", async () => {
    ...
    expect(screen.getByRole("link", { name: "[1] 넥쏘 2021 취급설명서" })).toHaveAttribute(
      "href",
  ...
  it("shows a grounded AI explanation before collapsible official evidence", async () => {
    globalThis.fetch = vi.fn()
      ...
      .mockResolvedValueOnce({
        ...
        status: 200,
        json: async () => ({
          answer: "부스트 모드는 가속을 위해 최대 10초 동안 모터 성능을 높입니다. [1]",
          sources: [{
            document_name: "ELECTRIFIED GV70 2027 취급설명서",
            source_url: "https://ownersmanual.genesis.com/manual/egv70-2027",
            page: 380,
            section: "부스트 모드",
            excerpt: "부스트 모드는 가속을 위해 최대 10초 동안 모터 장치의 최대 성능을 발휘합니다.",
          }],
          search_engine: "keyword-intent-v3",
    ...
    expect(await screen.findByText("AI 매뉴얼 검색 준비 완료")).toBeInTheDocument();
    fireEvent.change(screen.getByLabelText("설명서에서 찾을 내용"), {
      target: { value: "부스트 버튼은 뭐야?" },
    });
    fireEvent.click(screen.getByRole("button", { name: "설명서 검색" }));

    expect(await screen.findByText("AI 설명")).toBeInTheDocument();
    expect(screen.getByText("부스트 모드는 가속을 위해 최대 10초 동안 모터 성능을 높입니다. [1]")).toBeInTheDocument();
    expect(screen.getByText("공식 설명서 근거로 생성")).toBeInTheDocument();
    expect(screen.getByText(/AI 답변 근거/)).toBeInTheDocument();
    expect(screen.getByText("공식 원문 근거 1건 보기").closest("details")).not.toHaveAttribute("open");
  });
