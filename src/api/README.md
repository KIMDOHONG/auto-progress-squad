...
FastAPI와 SQLite 기반 백엔드입니다. 차량 프로필, 승인된 로컬 매뉴얼의 추출·청크 저장·출처 검색, 리콜 API 계약과 선택형 실제 경로 조회를 제공합니다. 생성형 매뉴얼 답변은 검증 환경에서만 선택적으로 연결합니다. 리콜은 자동차리콜센터 공급자 계약만 구현했으며 승인된 실제 HTTP 공급자는 아직 연결하지 않습니다.
...
| `APS_NAVER_MAPS_TIMEOUT_SECONDS` | `5` | Geocoding·Directions 5 요청 제한 시간(초) |
...
| `POST /api/v1/planner/route` | 주소 2개를 Geocoding한 뒤 Directions 5 `trafast`의 거리·예상 시간 반환; 공급자 미설정·주소 불일치·상류 장애 구분 |
...
- NAVER Maps Client ID와 Client Secret은 FastAPI 프로세스 환경변수에서만 읽습니다. 프런트엔드 응답·로그·저장소에는 인증값을 노출하지 않으며, Geocoding 또는 Directions 5가 미설정·실패한 경우 직접 입력 거리 계산으로 명시적으로 복귀합니다. 실제 계정 키를 사용한 라이브 E2E와 과금 확인은 아직 완료하지 않았습니다.
