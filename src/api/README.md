...
FastAPI와 SQLite 기반 백엔드입니다. 차량 프로필, 승인된 로컬 매뉴얼의 추출·청크 저장·출처 검색, 리콜 API 계약, 선택형 실제 경로 조회와 경로 주변 충전·주유소 공통 계약을 제공합니다. 생성형 매뉴얼 답변은 검증 환경에서만 선택적으로 연결합니다. 한국환경공단 전기차 충전소, 한국석유관리원 수소충전소, 한국석유공사 오피넷 주유소 공급자를 키가 설정된 경우에만 연결합니다.
...
| `APS_HYDROGEN_STATION_SERVICE_KEY` | 없음 | 공공데이터포털 수소충전소 운영·실시간정보 공용 서비스키 |
| `APS_OPINET_SERVICE_KEY` | 없음 | 오피넷 유가정보 API 인증키 |
| `APS_OPINET_CACHE_TTL_SECONDS` | `900` | 반경·상세 응답 메모리 캐시 시간(초) |
| `APS_OPINET_MAX_SAMPLE_POINTS` | `24` | 경로 한 번 조회에서 사용하는 최대 표본점 수 |
| `APS_OPINET_MAX_DETAIL_REQUESTS` | `15` | 후보 상세정보 최대 요청 수 |
...
- 충전·주유소 공통 계약은 검토용 `APS_STATION_CATALOG_PATH` 정규 JSON 스냅샷을 우선하고, 스냅샷이 없으면 설정된 키에 따라 전기·수소·주유소 공급자를 독립적으로 활성화합니다. 오피넷은 공식 최대 반경 5 km와 KATEC 좌표를 사용하며, 경로 표본점에서 수집한 후보를 WGS84로 변환해 다시 경로선 거리로 거릅니다. 일반휘발유(B027)·고급휘발유(B034)·자동차경유(D047)만 취급을 확인하고, 별도 공개 코드가 없는 초고급휘발유·하이세탄경유 및 API가 제공하지 않는 현재 영업 상태는 추정하지 않습니다. 경로와 후보 좌표의 근접도는 실제 도로 우회거리나 우회시간이 아닙니다. 데이터 형식과 실패 경계는 [ADR-0010](../../docs/decisions/0010-normalize-local-energy-station-snapshots.md)을 따릅니다.
