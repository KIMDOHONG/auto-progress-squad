...
> 현재 AI 코파일럿의 차량 식별과 프로필 관리는 제조사 공식 차량 데이터를 사용하는 규칙형 기능입니다. 실제 LLM 기반 자유 대화와 매뉴얼 RAG 검색은 아직 연결하지 않았습니다.
...
외부 지도·전기·수소 충전소·주유소 API, 리콜 데이터와 실제 매뉴얼 RAG 모델은 아직 연결하지 않았으며 화면에서 미연동 상태를 명시합니다. FastAPI 모드에서는 정확한 공식 매뉴얼이 확인된 차량마다 문서 준비 작업을 `확인 중(pending) → 사용 가능(ready) / 실패(failed)` 상태로 관리하고, 준비 전에는 RAG 검색을 차단하는 기반을 제공합니다. 실제 PDF 수집·텍스트 추출·검색 인덱스 생성 작업자는 다음 단계이며, 저작권·재배포 조건을 확인한 문서만 서버 저장소에 보관합니다. GitHub Pages나 브라우저 로컬 저장소에 제조사 PDF를 일괄 복제하지 않습니다.
...
공식 매뉴얼이 정확히 매칭된 차량은 등록과 동시에 SQLite에 `pending` 준비 작업이 생성됩니다. 매뉴얼 화면은 API 상태를 조회해 **취급설명서를 확인 중입니다**를 표시하며, `ready`가 되기 전에는 질문 전송을 막습니다. 현재 단계에서는 작업 상태와 재시도 계약만 구현되어 실제 문서 다운로드나 `ready` 전환은 수행하지 않습니다.
...
현재 백엔드는 차량 프로필 CRUD·활성 차량 전환용 SQLite API, 차량별 매뉴얼 준비 상태·재시도 API와 매뉴얼·리콜 계약을 제공합니다. 프런트엔드의 공식 취급설명서 링크는 제조사 원문을 새 탭으로 열며, 실제 매뉴얼 RAG와 리콜 원천 데이터는 아직 연결하지 않았습니다. 백엔드 매뉴얼 검색은 정확한 문서가 없거나 준비 중이면 `409`, 준비 실패면 `503`, 준비 완료 뒤 검색 서비스가 미연결된 상태면 `503`으로 구분합니다.
