...
| EV 충전 플래너 | 수동 거리 또는 선택형 NAVER 실제 경로 거리·배터리·SoC·전비·도착 최소 SoC 기반 도착 가능 여부와 필요 충전량 | 로컬 계산·경로 공급자 구현, 실시간 충전소 API 미연동 |
| 수소 충전 플래너 | 수동 거리 또는 선택형 NAVER 실제 경로 거리와 현재 주행가능거리 기반 도착 가능 여부·부족 거리 | 로컬 계산·경로 공급자 구현, 실시간 수소충전소 API 미연동 |
| 주유 경로 플래너 | 수동 거리 또는 선택형 NAVER 실제 경로 거리와 현재 주행가능거리 비교, 지정연료 분기 | 로컬 계산·경로 공급자 구현, 주유소 API 미연동 |
...
실제 경로 거리·예상 시간을 함께 시험하려면 NAVER Cloud Maps 애플리케이션에서 **Geocoding**과 **Directions 5**를 활성화한 뒤, 같은 터미널에서 인증값을 환경변수로 설정합니다. 인증값은 `.env`, 프런트엔드 코드 또는 Git에 넣지 않습니다.
...
- 주소 기반 실제 경로 거리·시간 조회: `POST /api/v1/planner/route`
- 테스트: `uv run pytest`
...
2. **NAVER Maps 라이브 검증**: 별도 발급 키로 Geocoding·Directions 5의 실제 주소 E2E와 계정 한도·비용을 확인하되 키는 서버 환경변수에만 보관
