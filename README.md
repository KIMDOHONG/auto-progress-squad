...
현재 최신 개발 내용은 [Pull Request #21](https://github.com/KIMDOHONG/auto-progress-squad/pull/21)에서 변경 파일과 테스트 결과를 함께 확인할 수 있습니다.
...
1. **PR #21 검토·병합**: 공식 차량 식별, 프로필 교체·삭제, BMW 동적 안내와 채팅 스크롤 수정 반영
2. **RAG 문서 수집 기반 구현**: 프로필별 문서 상태를 `확인 중 → 사용 가능 → 실패`로 관리하고 서버 저장·검색 인덱스 구성
3. **매뉴얼 질의응답 연결**: 검색된 문서의 위치와 출처를 함께 보여주는 답변 구현
4. **리콜 API 연결**: 활성 차량 기준 리콜 조회와 대시보드 알림 구현
