...
[🚗 자동진행단 공개 프런트엔드 실행하기](https://kimdohong.github.io/auto-progress-squad/)

[🛠️ 현재 진행 중인 개발 브랜치 보기](https://github.com/KIMDOHONG/auto-progress-squad/tree/feat/local-drive-planner-mvp)
...
| 수소 충전 플래너 | 정확한 주소 확인·GPS 출발지·편도/왕복·복수 NAVER 실제 경로 선택 또는 수동 거리와 현재 주행가능거리 기반 도착 가능 여부·부족 거리, 가득 충전 원칙 고정 안내 | 한국석유관리원 운영·실시간정보 결합 및 실제 승인키 E2E 완료 |
| 주유 경로 플래너 | 정확한 주소 확인·GPS 출발지·편도/왕복·복수 NAVER 실제 경로 선택 또는 수동 거리와 탱크 용량·현재 연료량·평균 연비·도착 희망 잔량·단가 기반 필요 주유량·비용 계산, 가득/필요량 선택 | 오피넷 반경·상세 API와 KATEC 좌표 변환 구현, 실제 발급키 화면 E2E 확인 대기 |
...
외부 경로 조회는 선택형 NAVER Maps 서버 어댑터, 정확한 주소·GPS 확인, 복수 경로 선택과 실제 좌표 경로 표시까지 구현했습니다. 2026-09-03 실제 계정 키로 Geocoding·Reverse Geocoding·Directions 5의 로컬 E2E를 확인했으며, 확인 완료된 주소는 경로 조회 때 문자열로 다시 검색하지 않고 확정 좌표를 사용합니다. Dynamic Map은 아직 활성화하지 않아 좌표 경로 SVG를 사용합니다. 충전·주유소 공통 계약과 로컬 정규 스냅샷에 더해 한국환경공단 전기차 충전소 어댑터와 한국석유관리원 수소충전소 운영·실시간정보 결합 어댑터를 연결했습니다. 2026-09-08에는 승인된 실제 계정키로 부산-인제 NAVER 경로 주변 수소충전소의 좌표·상태·압력·대기 대수·튜브트레일러 압력·갱신시각이 화면까지 전달되는 E2E를 완료했습니다. 상세 결과는 [수소충전소 실제 계정키 E2E 기록](docs/hydrogen-station-live-e2e.md)에 남겼습니다. 2026-09-09에는 발급된 오피넷 인증키를 보안 입력할 수 있도록 실행 절차를 확장하고, 경로 표본점 KATEC 반경 조회와 상세 주소·가격·가격 기준시각을 결합하는 공급자를 구현했습니다. 일반휘발유·고급휘발유·자동차경유만 공식 제품코드 기준으로 취급을 확인하며, 별도 공개 코드가 없는 초고급휘발유·하이세탄경유와 API가 제공하지 않는 현재 영업 상태는 추정하지 않습니다. 실제 발급키의 화면 E2E는 재실행 후 확인할 다음 단계입니다. 실제 리콜 데이터도 아직 연결하지 않았습니다. 리콜은 사전 협의가 필요한 자동차리콜센터 API만 승인 공급자로 정하고, 공급자 미설정·장애·0건을 서로 다른 상태로 반환하는 서버 계약을 구현했습니다. 공급자 후보는 제조사·차명·연식·세대·프로젝트 코드의 정규 키가 모두 일치할 때만 반환하며 인접 연식이나 세대를 대신 연결하지 않습니다. FastAPI 모드에서는 정확한 공식 매뉴얼이 확인된 차량마다 문서 준비 작업을 `확인 중(pending) → 사용 가능(ready) / 실패(failed)` 상태로 관리합니다. 서버 관리자가 승인한 manifest의 PDF/TXT만 추출·청크화해 SQLite에 저장하며, `ready`가 된 현재 차량 문서에서만 키워드 기반 출처 검색을 제공합니다. 제조사 사이트 자동 다운로드는 아직 수행하지 않고, GitHub Pages나 브라우저 로컬 저장소에도 제조사 PDF를 복제하지 않습니다.

### 2. FastAPI와 함께 로컬 실행

Windows에서는 저장소 루트에서 아래 단일 명령을 사용하는 방법을 권장합니다. NAVER Client ID·Client Secret, 선택형 EV 충전소 서비스키, 선택형 수소충전소 서비스키, 선택형 오피넷 인증키를 차례로 입력하면 FastAPI(`8000`)와 Vite(`5173`)가 함께 실행되고 브라우저가 열립니다. 활용신청이 아직 승인되지 않은 공공데이터는 해당 입력에서 바로 Enter를 눌러 건너뛸 수 있습니다. 시스템 PATH에 Node.js가 없어도 Codex 데스크톱에 포함된 Node.js를 자동으로 찾습니다. 키는 파일이나 명령행에 저장되지 않으며 이 PowerShell 창에서 `Ctrl+C`를 누르면 두 서버가 함께 종료됩니다.
...
1. **오피넷 실제 발급키 화면 E2E**: 구현된 반경·상세 공급자에 발급키를 보안 입력해 BMW M3 고급휘발유 경로에서 주소·가격·가격시각과 영업 상태 미제공 안내를 확인
...
| [docs/opinet-fuel-station-adapter.md](docs/opinet-fuel-station-adapter.md) | 오피넷 주유소 반경·상세 어댑터 구현과 검증 경계 |
...
| 이름 | GitHub | 담당 영역 |
| --- | --- | --- |
| 김도홍 | [@KIMDOHONG](https://github.com/KIMDOHONG) | 프로젝트 총괄, 프런트엔드·백엔드 통합, 차량 프로필·플래너, GitHub 형상관리·통합 검증 |
| 유성철 | - | 제조사 공식 취급설명서 자료 준비, RAG 인덱싱·질문 평가, 출처 기반 답변 품질 점검 |
| 김영수 | - | 공공데이터 공급자 조사·정규화, 충전·주유소 데이터 검증, 외부 API 테스트 자료 정리 |
