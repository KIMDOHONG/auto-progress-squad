...
| EV 충전 플래너 | 수동 거리·배터리·SoC·전비·도착 최소 SoC 기반 도착 가능 여부와 필요 충전량 | 로컬 계산 완료·지도/충전소 API 미연동 |
| 수소 충전 플래너 | 수동 거리와 현재 주행가능거리 기반 도착 가능 여부·부족 거리 | 로컬 계산 완료·실시간 충전소 API 미연동 |
| 주유 경로 플래너 | 수동 거리와 현재 주행가능거리 비교, 지정연료 분기 | 로컬 계산 완료·주유소 API 미연동 |
...
동력원별 플래너는 지도 API가 없어도 경로 거리를 직접 입력해 사용할 수 있습니다. EV는 배터리 용량·현재 SoC·전비·도착 최소 SoC로 예상 소비전력과 최소 필요 충전량을 계산하고, 수소·내연기관 차량은 현재 주행가능거리와 경로 거리를 비교합니다. 계산식과 입력 조건은 결과에 표시하며, 충전소·주유소 위치와 실시간 상태가 포함되지 않았다는 점을 별도 상태로 구분합니다.
...
1. **외부 경로 API 연결**: [Issue #5](https://github.com/KIMDOHONG/auto-progress-squad/issues/5)의 수동 거리 계산을 유지한 채, 로컬 FastAPI에서 실제 거리·시간을 조회하고 장애 시 직접 입력으로 복귀
2. **충전소·주유소 데이터 연결**: EV·수소·내연기관별 공식 데이터 어댑터와 경로 주변 후보 비교를 구현
