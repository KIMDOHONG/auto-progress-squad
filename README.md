...
| 매뉴얼·리콜 | 현대·기아·제네시스 공식 설명서 연결, BMW Driver's Guide 안내, FastAPI 모드의 승인된 쉐보레·KGM 장별 문서 수집 | 매뉴얼 연결·승인 문서 검색 구현·리콜 API 미연동 |
...
3. **생성형 매뉴얼 답변 실모델 검증**: 구현된 선택형 OpenVINO 답변·인용 검증 경계에 검토 모델을 연결하고 승인 문서에서 CPU 품질·지연·메모리 E2E 측정
