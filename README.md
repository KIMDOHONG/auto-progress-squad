...
수소차는 공공데이터포털에서 한국석유관리원 [수소충전소 운영정보](https://www.data.go.kr/data/15133332/openapi.do)와 [수소충전소 실시간정보](https://www.data.go.kr/data/15133338/openapi.do)를 둘 다 활용신청한 뒤 `APS_HYDROGEN_STATION_SERVICE_KEY`를 설정합니다. 어댑터는 관리번호로 두 응답을 결합해 위치·주소·영업 여부·충전기 규격·실시간 운영/POS/혼잡 상태·대기 차량 수·튜브트레일러 압력·갱신시각을 표시하며 기본 5분간 메모리에 캐시합니다. 영업정지·영업마감과 POS 영업중지·점검중·T/T 교체중·기타고장은 `이용 불가`, 혼잡 또는 대기 차량이 있으면 `혼잡`으로 보수적으로 변환합니다. 두 API 중 하나라도 권한이 없거나 장애가 있으면 실시간 상태를 임의로 추정하지 않고 `503`으로 구분합니다. 이 API는 무료·이용허락범위 제한 없음으로 공개되어 있지만, 현재 코드는 목 응답 테스트까지만 확인했고 실제 계정키 E2E는 두 활용신청 승인 후 확인해야 합니다.
...
1. **수소충전소 실제 계정키 E2E 확인**: 한국석유관리원 운영정보·실시간정보 활용신청이 둘 다 승인되면 실제 경로에서 좌표·상태·대기 대수·압력·갱신시각을 확인
