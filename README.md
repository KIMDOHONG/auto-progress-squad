...
| 차량 관리 | 차량 최대 3대 등록·수정·삭제, 활성 차량 전환, 필수 트림·세부 구동 사양, 승인된 전기차 배터리 제원 자동 적용 | 기반 구현 완료, 공식 제원 카탈로그 확대 필요 |
...
| EV 충전 플래너 | 정확한 주소 확인·GPS 출발지·편도/왕복·복수 NAVER 실제 경로 선택 또는 수동 거리와 배터리·SoC·전비·도착 최소 SoC 기반 도착 가능 여부·필요 충전량 | 로컬 계산·경로 공급자·좌표 경로 표시 구현, 실시간 충전소 API 미연동 |
| 수소 충전 플래너 | 정확한 주소 확인·GPS 출발지·편도/왕복·복수 NAVER 실제 경로 선택 또는 수동 거리와 현재 주행가능거리 기반 도착 가능 여부·부족 거리 | 로컬 계산·경로 공급자·좌표 경로 표시 구현, 실시간 수소충전소 API 미연동 |
| 주유 경로 플래너 | 정확한 주소 확인·GPS 출발지·편도/왕복·복수 NAVER 실제 경로 선택 또는 수동 거리와 현재 주행가능거리 비교, 지정연료 분기 | 로컬 계산·경로 공급자·좌표 경로 표시 구현, 주유소 API 미연동 |
...
- 새 프로필은 동력원·세부 구동 사양·트림을 반드시 확인하고, 정확히 승인된 전기차 구성은 배터리 용량과 공식 출처를 자동 저장
- 현재 사용할 차량 선택
...
차량 등록의 자동 제원 카탈로그는 연식·차명·동력원·세부 구동 사양·트림이 모두 일치할 때만 적용됩니다. 현재 승인 범위는 [2026 기아 EV6](https://kwp1.kia.com/content/dam/kwp/kr/ko/vehicles/pdf/catalog/catalog_ev6.pdf)와 [2027 기아 EV6](https://www.kia.com/kr/vehicles/ev6/price)(각각 스탠다드 2WD 63.0kWh, 롱레인지 2WD·4WD 84.0kWh), [2025 현대 코나 Electric](https://www.hyundai.com/contents/repn-car/catalog/kona-electric-2025-price.pdf)(스탠다드 48.6kWh, 롱레인지 64.8kWh), [2027 제네시스 ELECTRIFIED GV70](https://www.genesis.com/kr/ko/models/electrified-gv70)(AWD 듀얼 모터 84.0kWh)입니다. EV6는 스탠다드·롱레인지와 2WD·4WD를 하나의 세부 사양으로 선택하고, 그다음 실제 트림을 선택합니다. 자동 매핑이 없는 전기차는 트림과 세부 구동 사양 외에 사용자가 확인한 배터리 용량을 입력해야 하며, 이를 공식 자동 제원처럼 표시하지 않습니다. 출처는 각 프로필에 확인일과 함께 저장합니다.

동력원별 플래너는 지도 API가 없어도 경로 거리를 직접 입력해 사용할 수 있습니다. FastAPI 모드에서 서버에 NAVER Maps 인증값을 설정하면 출발지·목적지의 정확한 주소를 먼저 확인하거나 GPS 좌표를 도로명 주소로 변환한 뒤, 실시간 빠른 길·실시간 최적·무료 우선 경로의 거리·예상 시간·통행료와 실제 좌표 경로를 조회합니다. 편도와 왕복을 선택할 수 있으며 왕복은 가는 길과 오는 길을 각각 조회해 거리·시간·통행료·경로선을 합산합니다. 사용자가 경로를 바꾸면 해당 거리로 에너지 계산도 다시 수행하며, Web Dynamic Map 키가 없을 때도 좌표 경로 SVG를 표시합니다. 조회 실패는 0 km나 계산 성공으로 취급하지 않습니다. EV는 배터리 용량·현재 SoC·전비·도착 최소 SoC로 예상 소비전력과 최소 필요 충전량을 계산하고, 수소·내연기관 차량은 현재 주행가능거리와 경로 거리를 비교합니다. 계산식과 입력 조건은 결과에 표시하며, 충전소·주유소 위치와 실시간 상태가 포함되지 않았다는 점을 별도 상태로 구분합니다.
...
외부 경로 조회는 선택형 NAVER Maps 서버 어댑터, 정확한 주소·GPS 확인, 복수 경로 선택과 실제 좌표 경로 표시까지 구현했습니다. 2026-09-03 실제 계정 키로 Geocoding·Reverse Geocoding·Directions 5의 로컬 E2E를 확인했으며, 확인 완료된 주소는 경로 조회 때 문자열로 다시 검색하지 않고 확정 좌표를 사용합니다. Dynamic Map은 아직 활성화하지 않아 좌표 경로 SVG를 사용합니다. 전기·수소 충전소·주유소 API와 실제 리콜 데이터는 아직 연결하지 않았고 화면에서 미연동 상태를 명시합니다. 리콜은 사전 협의가 필요한 자동차리콜센터 API만 승인 공급자로 정하고, 공급자 미설정·장애·0건을 서로 다른 상태로 반환하는 서버 계약을 구현했습니다. 공급자 후보는 제조사·차명·연식·세대·프로젝트 코드의 정규 키가 모두 일치할 때만 반환하며 인접 연식이나 세대를 대신 연결하지 않습니다. FastAPI 모드에서는 정확한 공식 매뉴얼이 확인된 차량마다 문서 준비 작업을 `확인 중(pending) → 사용 가능(ready) / 실패(failed)` 상태로 관리합니다. 서버 관리자가 승인한 manifest의 PDF/TXT만 추출·청크화해 SQLite에 저장하며, `ready`가 된 현재 차량 문서에서만 키워드 기반 출처 검색을 제공합니다. 제조사 사이트 자동 다운로드는 아직 수행하지 않고, GitHub Pages나 브라우저 로컬 저장소에도 제조사 PDF를 복제하지 않습니다.
...
Windows 로컬 실행에서는 인증값을 파일에 남기지 않는 도우미를 사용할 수 있습니다. 두 값을 차례로 붙여 넣으면 백엔드가 기본 `http://127.0.0.1:8001`에서 실행되며, 종료하면 프로세스 환경변수도 제거됩니다.
...
1. **충전소·주유소 데이터 연결**: [Issue #5](https://github.com/KIMDOHONG/auto-progress-squad/issues/5)의 EV·수소·내연기관별 공식 데이터 어댑터와 경로 주변 후보 비교를 구현
2. **NAVER Maps 라이브 검증**: 별도 발급 키로 Geocoding·Reverse Geocoding·Directions 5·Dynamic Map의 실제 주소/GPS E2E와 계정 한도·비용을 확인하되 서버 Secret은 서버 환경변수에만 보관
...
4. **검색·생성 품질 실제 문서 평가**: 선택형 OpenVINO 검색·생성을 승인된 실제 문서와 확장 질문 세트로 재평가
