...
| EV 충전 플래너 | 정확한 주소 확인·GPS 출발지·복수 NAVER 실제 경로 선택 또는 수동 거리와 배터리·SoC·전비·도착 최소 SoC 기반 도착 가능 여부·필요 충전량 | 로컬 계산·경로 공급자·좌표 경로 표시 구현, 실시간 충전소 API 미연동 |
| 수소 충전 플래너 | 정확한 주소 확인·GPS 출발지·복수 NAVER 실제 경로 선택 또는 수동 거리와 현재 주행가능거리 기반 도착 가능 여부·부족 거리 | 로컬 계산·경로 공급자·좌표 경로 표시 구현, 실시간 수소충전소 API 미연동 |
| 주유 경로 플래너 | 정확한 주소 확인·GPS 출발지·복수 NAVER 실제 경로 선택 또는 수동 거리와 현재 주행가능거리 비교, 지정연료 분기 | 로컬 계산·경로 공급자·좌표 경로 표시 구현, 주유소 API 미연동 |
...
동력원별 플래너는 지도 API가 없어도 경로 거리를 직접 입력해 사용할 수 있습니다. FastAPI 모드에서 서버에 NAVER Maps 인증값을 설정하면 출발지·목적지의 정확한 주소를 먼저 확인하거나 GPS 좌표를 도로명 주소로 변환한 뒤, 실시간 빠른 길·실시간 최적·무료 우선 경로의 거리·예상 시간·통행료와 실제 좌표 경로를 조회합니다. 사용자가 경로를 바꾸면 해당 거리로 에너지 계산도 다시 수행하며, Web Dynamic Map 키가 없을 때도 좌표 경로 SVG를 표시합니다. 조회 실패는 0 km나 계산 성공으로 취급하지 않습니다. EV는 배터리 용량·현재 SoC·전비·도착 최소 SoC로 예상 소비전력과 최소 필요 충전량을 계산하고, 수소·내연기관 차량은 현재 주행가능거리와 경로 거리를 비교합니다. 계산식과 입력 조건은 결과에 표시하며, 충전소·주유소 위치와 실시간 상태가 포함되지 않았다는 점을 별도 상태로 구분합니다.
...
실제 경로 거리·예상 시간·경로선을 함께 시험하려면 NAVER Cloud Maps 애플리케이션에서 **Geocoding**, **Reverse Geocoding**, **Directions 5**, **Dynamic Map**을 활성화합니다. 서버 호출용 Client ID·Client Secret과 웹 지도용 Client ID를 환경변수로 설정합니다. 서버 Client Secret은 `.env`, 프런트엔드 코드, 로그 또는 Git에 넣지 않습니다. 웹 지도 Client ID는 브라우저에 전달되는 공개 식별자이므로 NAVER Cloud 콘솔에서 `http://127.0.0.1:5173` 등 실제 사용하는 Web 서비스 URL만 허용해야 합니다.
