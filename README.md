...
외부 경로 조회는 선택형 NAVER Maps 서버 어댑터, 정확한 주소·GPS 확인, 복수 경로 선택과 실제 좌표 경로 표시까지 구현했습니다. 2026-09-03 실제 계정 키로 Geocoding·Reverse Geocoding·Directions 5의 로컬 E2E를 확인했으며, 확인 완료된 주소는 경로 조회 때 문자열로 다시 검색하지 않고 확정 좌표를 사용합니다. Dynamic Map은 아직 활성화하지 않아 좌표 경로 SVG를 사용합니다. 전기·수소 충전소·주유소는 출처와 조회시각이 있는 로컬 정규 스냅샷을 읽어 경로 반경 후보를 반환하고 화면에서 비교하는 공통 계약까지 구현했으며, 실제 공급자 HTTP API는 아직 연결하지 않아 미연동 상태를 유지합니다. 실제 리콜 데이터도 아직 연결하지 않았습니다. 리콜은 사전 협의가 필요한 자동차리콜센터 API만 승인 공급자로 정하고, 공급자 미설정·장애·0건을 서로 다른 상태로 반환하는 서버 계약을 구현했습니다. 공급자 후보는 제조사·차명·연식·세대·프로젝트 코드의 정규 키가 모두 일치할 때만 반환하며 인접 연식이나 세대를 대신 연결하지 않습니다. FastAPI 모드에서는 정확한 공식 매뉴얼이 확인된 차량마다 문서 준비 작업을 `확인 중(pending) → 사용 가능(ready) / 실패(failed)` 상태로 관리합니다. 서버 관리자가 승인한 manifest의 PDF/TXT만 추출·청크화해 SQLite에 저장하며, `ready`가 된 현재 차량 문서에서만 키워드 기반 출처 검색을 제공합니다. 제조사 사이트 자동 다운로드는 아직 수행하지 않고, GitHub Pages나 브라우저 로컬 저장소에도 제조사 PDF를 복제하지 않습니다.
