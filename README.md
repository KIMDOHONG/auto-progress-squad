...
외부 경로 조회는 선택형 NAVER Maps 서버 어댑터, 정확한 주소·GPS 확인, 복수 경로 선택과 실제 좌표 경로 표시까지 구현했습니다. 2026-09-03 실제 계정 키로 Geocoding·Reverse Geocoding·Directions 5의 로컬 E2E를 확인했으며, 확인 완료된 주소는 경로 조회 때 문자열로 다시 검색하지 않고 확정 좌표를 사용합니다. Dynamic Map은 아직 활성화하지 않아 좌표 경로 SVG를 사용합니다. 충전·주유소 공통 계약과 로컬 정규 스냅샷에 더해 [한국환경공단 전기자동차 충전소 정보](https://www.data.go.kr/data/15013115/standard.do)의 위치·출력·상태 HTTP 어댑터를 연결했습니다. FastAPI 실행 시 공공데이터 서비스키가 설정된 경우에만 EV 후보를 조회하며, 수소충전소·주유소는 아직 공급자 미설정 상태를 유지합니다. 실제 리콜 데이터도 아직 연결하지 않았습니다. 리콜은 사전 협의가 필요한 자동차리콜센터 API만 승인 공급자로 정하고, 공급자 미설정·장애·0건을 서로 다른 상태로 반환하는 서버 계약을 구현했습니다. 공급자 후보는 제조사·차명·연식·세대·프로젝트 코드의 정규 키가 모두 일치할 때만 반환하며 인접 연식이나 세대를 대신 연결하지 않습니다. FastAPI 모드에서는 정확한 공식 매뉴얼이 확인된 차량마다 문서 준비 작업을 `확인 중(pending) → 사용 가능(ready) / 실패(failed)` 상태로 관리합니다. 서버 관리자가 승인한 manifest의 PDF/TXT만 추출·청크화해 SQLite에 저장하며, `ready`가 된 현재 차량 문서에서만 키워드 기반 출처 검색을 제공합니다. 제조사 사이트 자동 다운로드는 아직 수행하지 않고, GitHub Pages나 브라우저 로컬 저장소에도 제조사 PDF를 복제하지 않습니다.

### 2. FastAPI와 함께 로컬 실행

Windows에서는 저장소 루트에서 아래 단일 명령을 사용하는 방법을 권장합니다. NAVER Client ID·Client Secret과 선택형 공공데이터포털 서비스키를 차례로 입력하면 FastAPI(`8000`)와 Vite(`5173`)가 함께 실행되고 브라우저가 열립니다. 전기차 충전소 키가 아직 없으면 세 번째 입력에서 바로 Enter를 눌러 건너뛸 수 있습니다. 시스템 PATH에 Node.js가 없어도 Codex 데스크톱에 포함된 Node.js를 자동으로 찾습니다. 키는 파일이나 명령행에 저장되지 않으며 이 PowerShell 창에서 `Ctrl+C`를 누르면 두 서버가 함께 종료됩니다.
...
플래너 화면은 실제 경로를 선택한 뒤 충전소·수소충전소·주유소 후보를 경로 반경별로 비교 표시합니다. 전기차는 `APS_EV_CHARGER_SERVICE_KEY`가 있으면 한국환경공단 공식 API를 사용하고, 같은 충전소의 여러 충전기는 한 후보로 묶어 최대 출력·통합 상태·가장 최근 상태시각을 표시합니다. 전국 응답은 기본 30분간 메모리에 캐시하며 `APS_EV_CHARGER_REGION_CODES=26,48`처럼 시도 코드를 지정하면 수집 범위를 제한할 수 있습니다. 후보의 경로 근접 거리는 도로 우회거리가 아닌 경로선과의 직선거리이고 대기시간은 제공되지 않습니다. 실제 공급자가 설정되지 않은 동력원은 계산 결과를 유지한 채 `공급자 미설정`으로 안내합니다. 검토용 로컬 스냅샷은 `APS_STATION_CATALOG_PATH`로 지정할 수 있으며, 이 설정이 공식 EV 어댑터보다 우선합니다. API 키나 원본 응답은 저장소에 포함하지 않습니다.
...
1. **수소충전소 실제 공급자 연결**: [Issue #5](https://github.com/KIMDOHONG/auto-progress-squad/issues/5)의 공통 계약과 구현된 한국환경공단 EV 어댑터 뒤에 수소충전소 공식 수집 어댑터를 연결
